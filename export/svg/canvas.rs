...
use microcad_core::{
    Bounds2D, Circle, Edge2D, Geometry2D, LineString, MultiLineString, Point, Polygon, Rect,
...
impl MapToCanvas for (Scalar, Scalar) {
    fn map_to_canvas(&self, canvas: &Canvas) -> Self {
        ...
        let y = canvas.content_rect.max().y - self.1 - canvas.content_rect.min().y; // Flip Y
