...
fn difference() -> Symbol {
    Symbol::new_builtin_module("difference", &|_, _| Ok(algorithm::difference()))
}

/// Creates a node containing a union algorithm
fn union() -> Symbol {
    Symbol::new_builtin_module("union", &|_, _| Ok(algorithm::union()))
}

/// Creates a node containing an intersection algorithm
fn intersection() -> Symbol {
    Symbol::new_builtin_module("intersection", &|_, _| Ok(algorithm::intersection()))
}

/// Creates a node containing a complement algorithm
fn complement() -> Symbol {
    Symbol::new_builtin_module("complement", &|_, _| Ok(algorithm::complement()))
}
